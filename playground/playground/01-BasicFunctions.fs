...
module BasicFunctions =
    ...
    let ex1() = printfn $"The result of squaring the integer 4573 and adding 3 is %d{result1}"

    /// When needed, annotate the type of a parameter name using '(argument:type)'.  Parentheses are required.
    let sampleFunction2 (x:int) = 2*x*x - x/5 + 3

    let result2 = sampleFunction2 (7 + 4)
    let ex2() = printfn $"The result of applying the 2nd sample function to (7 + 4) is %d{result2}"
    ...
    let ex3() = printfn $"The result of applying the 3rd sample function to (6.5 + 4.5) is %f{result3}"
